...
def compute_ap(gt_boxes, gt_class_ids, gt_masks,
    ...
    for i, class_id in enumerate(pred_class_ids):
        ...
        recalls_math = np.concatenate([[0], [recalls_for_classAP[i]], [1]])

        for j in range(len(precisions_math) - 2, -1, -1):
            # Ensure precision values decrease but don't increase. This way, the
            # precision value at each recall threshold is the maximum it can be
            # for all following recall thresholds, as specified by the VOC paper.
            if j + 1 < len(precisions_math):
                precisions_math[j] = np.maximum(precisions_math[j], precisions_math[j + 1])

        # Compute mean AP over recall range
        indices = np.where(recalls_math[:-1] != recalls_math[1:])[0] + 1
